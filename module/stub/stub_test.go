...
func TestNew(t *testing.T) {
	...
	if mi == nil {
		t.Fatal("Could not create internal module")
	}
	if mi.ID() != mid {
		t.Fatalf("Inconsistent MID for internal module, expected: %s, actual: %s", mid, mi.ID())
	}
	if mi.Addr() != addrStr {
		t.Fatalf("Inconsistent addr for internal module, expected: %s, actual: %s", addrStr, mi.Addr())
	}
	if mi.Score() != 0 {
		t.Fatalf("Inconsistent score for internal module, expected: %d, actual: %d", 0, mi.Score())
	}
	...
	if mi.AcceptedCount() != 0 {
		t.Fatalf("Inconsistent accepted count for internal module, expected: %d, actual: %d", 0, mi.AcceptedCount())
	}
	if mi.CompletedCount() != 0 {
		t.Fatalf("Inconsistent completed count for internal module, expected: %d, actual: %d", 0, mi.CompletedCount())
	}
	if mi.HandlingNumber() != 0 {
		t.Fatalf("Inconsistent handling number for internal module, expected: %d, actual: %d", 0, mi.HandlingNumber())
	}
	illegalMID := module.MID("D127.0.0.1")
	_, err = NewModuleInternal(illegalMID, nil)
	if err == nil {
...
func TestScore(t *testing.T) {
	...
	for i := uint64(1); i < number; i++ {
		...
		score := mi.Score()
		if score != i {
			t.Fatalf("Inconsistent socre for internal moduel, expected: %d, actual %d", i, score)
		}
	}
}
...
func TestCalledCount(t *testing.T) {
	...
	for i := uint64(1); i < number; i++ {
		mi.IncrCalledCount()
		if mi.CalledCount() != i {
			t.Fatalf("Inconsistent called count for internal module, expected: %d, actual: %d", i, mi.CalledCount())
		}
	}
}
...
func TestAcceptedCount(t *testing.T) {
	...
	for i := uint64(1); i < number; i++ {
		...
		if mi.AcceptedCount() != i {
			t.Fatalf("Inconsistent accepted count for internal module, expected: %d, actual: %d", i, mi.AcceptedCount())
		}
	}
}
func TestCompletedCount(t *testing.T) {
	...
	for i := uint64(1); i < number; i++ {
		...
		if mi.CompletedCount() != i {
			t.Fatalf("Inconsistent completed count for internal module, expected: %d, actual: %d", i, mi.CompletedCount())
		}
	}
}
...
func TestHandlingNumber(t *testing.T) {
	...
	for i := uint64(1); i < number; i++ {
		...
		if mi.HandlingNumber() != i+2 {
			t.Fatalf("Inconsistent handling number for internal module, expected: %d, actual: %d", i+2, mi.HandlingNumber())
		}
		mi.DecrHandlingNumber()
		mi.DecrHandlingNumber()
		if mi.HandlingNumber() != i {
			t.Fatalf("Inconsistent handling number for internal module, expected: %d, actual: %d", i, mi.HandlingNumber())
		}
	}
}
...
func TestAllInParallel(t *testing.T) {
	...
	t.Run("HandingNumber in Parallel", func(t *testing.T) {
		t.Parallel()
		for i := uint64(1); i < number; i++ {
			mi.IncrHandlingNumber()
			mi.IncrHandlingNumber()
