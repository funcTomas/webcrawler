...
func TestMIDGenAndSplit(t *testing.T) {
	...
	for _, addr := range addrs {
		for _, mt := range legalTypes {
			expectSN := DefaultSNGen.Get()
			mid, err := GenMID(mt, expectSN, addr)
			if err != nil {
				t.Fatalf("an error occurs when generating module ID: %s (type: %s, sn: %d, addr: %s)",
					err, mt, expectSN, addr)
			}
			...
			if letter != expectedLetter {
				t.Fatalf("Inconsistent type letter in MID, expected: %s, actual: %s", expectedLetter, letter)
			}
			...
			if sn != expectSN {
				t.Fatalf("Inconsistent sn in MID, expected: %d, actual: %d", expectSN, sn)
			}
			if addrStr != expectedAddrStr {
				t.Fatalf("Inconsistent address in MID, expected: %s, acutal: %s", expectedAddrStr, addrStr)
			}
		}
	}
	for _, addr := range addrs {
		for _, mt := range illegalTypes {
			...
			if err == nil {
				t.Fatalf("It still can generate module ID with illegal type %q", mt)
			}
			if string(mid) != "" {
				t.Fatalf("It still can generate module ID %q with illegal type %q", mid, mt)
			}
		}
	}
	for _, illegalMID := range illegalMIDs {
		if _, err := SplitMID(illegalMID); err == nil {
			t.Fatalf("It still can split illegal moduel ID %q", illegalMID)
		}
	}
}
...
func TestMIDLegal(t *testing.T) {
	...
	for _, mt := range legalTypes {
		for mip := range legalIPMap {
			...
			if !LegalMID(mid) {
				t.Fatalf("The generated MID %q is legal, but not be detected", mid)
			}
		}
	}
	for _, illegalMID := range illegalMIDs {
		if LegalMID(illegalMID) {
			t.Fatalf("The generated MID %q is illegal, but not detected", illegalMID)
		}
	}
}
